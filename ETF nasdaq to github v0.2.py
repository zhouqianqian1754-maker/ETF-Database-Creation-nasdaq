...
def get_hist_folder_and_file_path(ticker: str):
    ...
    hist_folder = ticker_dir / f"{ticker}_hist_till{current_date}"
    hist_folder.mkdir(parents=True, exist_ok=True)

    file_path = hist_folder / f"{ticker}_hist_till{current_date}_{current_time}.csv"
    return ticker_dir, hist_folder, file_path


def get_hist_file_date(file_path: Path):
    stem = file_path.stem
    suffix = stem.split("_hist_till", 1)[1]
...
def process_ticker(ticker: str):
    ...
    ticker_dir, hist_folder, new_file_path = get_hist_folder_and_file_path(ticker)
    ...
    hist_files = sorted(
        ticker_dir.rglob(f"{ticker}_hist_till*.csv"),
        key=get_hist_file_date
    )
